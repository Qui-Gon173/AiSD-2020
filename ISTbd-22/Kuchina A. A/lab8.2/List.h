...
void List::creation_node_in_target_place(int place,unsigned long field_data)
{
    ...
    current->pPrevious = conteiner_for_uk;//исправляем указатель на предыдущий
    };
    size++;
...
void List::delete_target_element(int target)
{
    ...
    current->pPrevious = conteiner_for_uk;//переназначаем указатель на предыдущий с удаляемого элемента
    delete uk_to_delete;
    }
    else
        {
