...
public:
    ...
    Iterator end() { return tail->ptr_next; }
...
public:
    ...
    /*friend ostream& operator<<(ostream& os, const DoubleList<T>::Node<T>& obj)
    {
        os << (int)obj.data;
        return os;
    }*/
...
int main()
{
    ...
    cout << "Минимальное значение: " << min.data << endl;

    auto max = max_element(byteList.begin(), byteList.end());
    cout << "Максимальное значение: " << max.data << endl;
