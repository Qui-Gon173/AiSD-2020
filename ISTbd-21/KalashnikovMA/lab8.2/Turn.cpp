...
void Turn::generate(unsigned long firstElem, unsigned long q, int count)
{
	// для функции pushArr
	unsigned long* arrToPush = new unsigned long[count];

	// создаём новую последовательность
	// её эл-ы добавляем в массив
	Generator sequence(firstElem, q, count);
	int i = 0;
	for (Generator::iterator item = sequence.begin(); item != sequence.end(); ++item)
	{
		arrToPush[i] = *item;
		i++;
	}
